...
export class MentionDirective {
  ...
  keyHandler(event: any, nativeElement: HTMLInputElement = this._element.nativeElement) {
    ...
    let lastCadena = nativeElement.value;
    let charPressed = lastCadena.charAt(lastCadena.length-1);
    if (!charPressed) {
    ...
    let cadena = nativeElement.value;
    console.log("valor",cadena.charAt(cadena.length-1));
    if (this.triggerChar === cadena.charAt(cadena.length-1)) {
      this.startPos = pos;
      ...
      if (event.keyCode !== KEY_SHIFT &&
        ...
        if (event.keyCode === KEY_LEFT || event.keyCode === KEY_RIGHT) {
          ...
          // update search
          let mention = val.substring(this.startPos );
          if (event.keyCode !== KEY_BACKSPACE) {
            mention.concat(charPressed) ;
